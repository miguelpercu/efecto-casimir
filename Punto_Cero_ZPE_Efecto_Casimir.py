...
# =============================================================================
# COMPLETE ZPE & CASIMIR EFFECT ANALYSIS - UAT/LRCP FRAMEWORK (REFINED)
# =============================================================================
...
class CausalUniverseConstants:
    ...
    def __init__(self):
        ...
        # Create results directory using best practice
        self.results_dir = "ZPE"
...
# =================================================================
# 4. COMPREHENSIVE VISUALIZATION (REFINED WITH OS.PATH.JOIN)
# =================================================================
...
class CausalVisualizer:
    ...
    def plot_zpe_cutoff_dependence(self, frequencies, zpe_densities, nu_planck):
        ...
        # Save plot using os.path.join for cross-platform compatibility
        plot_path = os.path.join(self.CONST.results_dir, "zpe_cutoff_dependence.png")
    ...
    def plot_causal_regulation_process(self, zpe_results):
        ...
        # Save plot using os.path.join
        plot_path = os.path.join(self.CONST.results_dir, "causal_regulation_process.png")
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_casimir_force_analysis(self, casimir_results):
        ...
        # Save plot using os.path.join
        plot_path = os.path.join(self.CONST.results_dir, "casimir_force_analysis.png")
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.show()
...
# =================================================================
# 5. SCIENTIFIC DOCUMENTATION AND REPORT GENERATION (REFINED)
# =================================================================
...
"""
        ...
        file_path = os.path.join(self.CONST.results_dir, "executive_summary.txt")
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(summary)

        print(f"✓ Executive summary generated: {file_path}")
        return file_path
...
"""
        ...
        # Use os.path.join for cross-platform compatibility
        file_path = os.path.join(self.CONST.results_dir, "technical_report.txt")
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(report)

        print(f"✓ Technical report generated: {file_path}")
        return file_path
    ...
    def generate_csv_data(self):
        ...
        # Use os.path.join for cross-platform compatibility
        main_csv_path = os.path.join(self.CONST.results_dir, "zpe_main_results.csv")
        ...
        # Use os.path.join for cross-platform compatibility
        casimir_csv_path = os.path.join(self.CONST.results_dir, "casimir_force_data.csv")
        df_casimir = pd.DataFrame(casimir_data)
        df_casimir.to_csv(casimir_csv_path, index=False)

        print(f"✓ CSV data generated: {main_csv_path}, {casimir_csv_path}")
        return main_csv_path, casimir_csv_path

# =================================================================
# 6. MAIN EXECUTION ENGINE (REFINED)
# =================================================================
...
def main():
    ...
    # Initialize framework (directory creation handled in CausalUniverseConstants)
    ...
    plot1_path = visualizer.plot_zpe_cutoff_dependence(frequencies, zpe_densities, constants.nu_PLANCK)
    plot2_path = visualizer.plot_causal_regulation_process(zpe_results)
    plot3_path = visualizer.plot_casimir_force_analysis(full_results)

    # Generate documentation
    print("\nGENERATING SCIENTIFIC DOCUMENTATION...")
    docs = ScientificDocumentation(full_results, constants)
    summary_path = docs.generate_executive_summary()
    report_path = docs.generate_technical_report()
    csv_paths = docs.generate_csv_data()
    ...
    print(f"\nGENERATED FILES IN '{constants.results_dir}' DIRECTORY:")
    print(f"  📊 {os.path.basename(summary_path)}")
    print(f"  📊 {os.path.basename(report_path)}") 
    print(f"  📊 {os.path.basename(csv_paths[0])}")
    print(f"  📊 {os.path.basename(csv_paths[1])}")
